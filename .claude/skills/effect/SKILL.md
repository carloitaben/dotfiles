...
- Model records with `Schema.Struct(...)` plus a same-name `type` alias.
...
- Ordinary object record: `Schema.Struct(...)` plus same-name `type` alias.
...
- Reusable boundary-crossing tagged variant: `Schema.TaggedStruct(...)` plus same-name `type` alias.
...
- Service boundary: `Context.Service<Service, Shape>()(...)` plus `Layer.effect(...)` plus `Service.of(...)`.
